...
BINOP_SYMBOLS = {Add:'+',Sub:'-',Mult:'*',Div:'/',FloorDiv:'//',Mod:'%',LShift:'<<',RShift:'>>',BitOr:'|',BitAnd:'&',BitXor:'^',Pow:'**'} 
...
class SourceGenerator(NodeVisitor):
    ...
    def visit_arguments(self, node):
        ...
        return self.write(sep=', ', *args)
     
    def decorators(self, node):
        self.write(prepend='@', endeach='\n', sep='', *node.decorator_list)
     
    def visit_Assign(self, node):
        self.write(sep=', ', *node.targets) 
    ...
    def visit_Import(self, node):
        self.write(sep=',', start='import', prepend=' ', *node.names)
    ...
    def visit_Delete(self, node):
        self.write(sep=',', start='del', prepend=' ', *node.targets)
    ...
    def visit_Try(self, node):
        ...
        self.write(prepend='\n', *node.handlers + [('finally', node.finalbody)])
     
    def write_list(self, start, args):
        self.write(start=start, sep=',', prepend=' ', *args)
    ...
    def visit_Tuple(self, node):
        self.write(start='(', sep=', ', *node.elts) 
        if (len(node.elts) == 1):
            self.write(',') 
        self.write(')')
    ...
    def visit_BoolOp(self, node):
        self.write(sep=' ' + BOOLOP_SYMBOLS[type(node.op)] + ' ', *node.values, **enclose('()'))
     
    def visit_Compare(self, node):
        row = combine([node.left] + node.comparators, [CMPOP_SYMBOLS[type(op)] for op in node.ops]) 
        self.write(start='(', end=')', *row)
    ...
    def visit_comprehension(self, node):
        self.write('for', node.target, 'in', node.iter) 
        self.write(prepend=' if ', *node.ifs)
